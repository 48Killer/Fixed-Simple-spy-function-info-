...
function newRemote(type, data)
    ...
    local ColorBar = Create("Frame",{Name = "ColorBar",Parent = RemoteTemplate,BackgroundColor3 = (type == "event" and Color3.fromRGB(255, 242, 0)) or (type == "unreliableevent" and Color3.fromRGB(255, 165, 0)) or  Color3.fromRGB(99, 86, 245),BorderSizePixel = 0,Position = UDim2.new(0, 0, 0, 1),Size = UDim2.new(0, 7, 0, 18),ZIndex = 2})
...
function remoteHandler(data)
    ...
    elseif data.remote:IsA("UnreliableRemoteEvent") and lower(data.method) == "fireserver" then
        newRemote("unreliableevent", data)
